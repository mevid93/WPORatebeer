...
describe "BeermappingApi" do
  ...
  it "When HTTP GET returns zero entries, empty array is returned" do
    ...
    END_OF_STRING
    stub_request(:get, /.*loviisa/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })
    places = BeermappingApi.places_in("loviisa")
  ...
  it "When HTTP GET returns multiple entries, they are parsed and returned" do
    ...
    place = places[1] # get second entry
  ...
  describe "in case of a cache hit" do
    before :each do
      Rails.cache.clear
    end
